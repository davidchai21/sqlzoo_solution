...
SELECT title, name
  FROM movie JOIN(casting JOIN actor ON (actor.id=casting.actorid)) ON (movie.id=casting.movieid)
...
SELECT title, name
  ...
  WHERE movieid IN
  	(SELECT movieid FROM casting JOIN actor ON (actor.id=casting.actorid) WHERE actor.name='Julie Andrews')
  	AND ord=1;
